...
// Marmot libraries it links. BuildConfig.h derives MIDORI_DEBUG_LEVEL from
...
// targets once compiled at a different level from the library because they never
...
// comes from a function defined out of line in MarmotCommon, compiled with the
...
TEST_CASE("Test translation units see the libraries' debug level", "[build][abi]")
{
	CHECK(MidoriBuild::LibraryDebugLevel() == MIDORI_DEBUG_LEVEL);
}

TEST_CASE("Test translation units see the libraries' MidoriValue layout", "[build][abi]")
{
	CHECK(MidoriValue::LibrarySize() == sizeof(MidoriValue));
}

TEST_CASE("Test translation units see the libraries' version string", "[build][abi]")
{
	CHECK(MidoriBuild::LibraryVersionString() == std::string_view(MIDORI_VERSION_STRING));
}

TEST_CASE("Test translation units see the libraries' endianness", "[build][abi]")
{
