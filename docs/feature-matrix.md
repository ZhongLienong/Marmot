...
| Native libraries (`foreign ... from "library"`, `package.marmot` `[ffi]`) | Experimental | `compiler/tests/utility/NativeLibraryTests.cpp`, `tool/src/tests.rs`, `scripts/check_cli_contracts.py` | Source names the library; the `.mmc` records it. `marmotc run` finds and loads it before the program starts through the plan, `--library-path`, `MARMOT_LIBRARY_PATH` or the declaring module's directory; `check` and `build` never do. |
